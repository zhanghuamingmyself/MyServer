...
import com.zhanghuaming.myserver.util.ZipExtractorTask;
...
public class ApkFragment extends ListFragment implements ServiceConnection {
    ...
    private String path = Constant.pluginPath;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new ArrayAdapter<ApkItem>(getActivity(), 0) {
            ...
            public View getView(final int position, View convertView, final ViewGroup parent) {
                ...
                try {
                    if (item.installing) {
                        MySharedPreferences.save(getContext(),item.title.toString(),item.packageInfo.packageName);
                        btn.setText("安装中ing");
                    } else {
                    ...
                    if (files != null) {
                        for (File apk : files) {
                            if (apk.exists() && apk.getPath().toLowerCase().endsWith(".apk")) {
                                apks.add(apk);
                                SLog.i(TAG, "在自定义目录找到了-----" + apk.toString());
                            }
                        }
                    }
                }

                file = new File(Environment.getExternalStorageDirectory(), "PluginsDownload");
                if (file.exists() && file.isDirectory()) {
        ...
        if (v.getId() == R.id.button2) {
            ...
            try {
                if (PluginManager.getInstance().getPackageInfo(item.packageInfo.packageName, 0) != null) {
                    ...
                    final String z = Constant.assetsPath + item.packageInfo.packageName + ".zip";
                    String p = Constant.assetsPath;
                    ZipExtractorTask task = new ZipExtractorTask(z, p, getActivity(), true);
                    try {
                        task.execute();
                    } catch (Exception e) {
                        e.printStackTrace();
